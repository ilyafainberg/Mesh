...
- Repeatedly failing device-sync snapshot requests are the narrow poison-message exception. Because they are idempotent control requests, the relay acknowledges them instead of delivering them again after one failed attempt; ordinary envelopes and sync operations are unaffected.
