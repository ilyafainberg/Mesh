...
[TestClass]
[DoNotParallelize]
public sealed class ReplicationProfileStateTests
{
    ...
    public void ContactMerge_PreservesTokensAndRetainedMemberships()
    {
        ...
        var merged = ProfileProjection.MergeContact(existing, incoming, ["Friends", "Deleted"]);
    ...
    public void Projection_PreservesUnresolvedMembershipForRetrySafety()
    {
        ...
        var projected = ProfileProjection.Snapshot(profile);
    ...
    public void GuestAuthorization_IgnoresUnresolvedMembershipUntilCircleExists()
    {
        ...
        Assert.AreEqual(0, ProfileProjection.ResolveGuestCircles(contact, []).Count);
        CollectionAssert.AreEqual(
            new[] { "Friends" },
            ProfileProjection.ResolveGuestCircles(
                contact,
    ...
    public void CapabilityAudience_SplitStorageRoundTripsEveryVisibilitySurface()
    {
        ...
        using (var db = MeshDb.Open(databasePath, key))
        {
            ...
            db.UpsertAsset(skill.Record, skill.Content);
            db.UpsertAsset(knowledge.Record, knowledge.Content);
            db.UpsertAsset(widget.Record, widget.Content);
    ...
    public void RecreatedCircle_CanBeJoinedOnlyAfterNewerUpsertAndLaterContact()
    {
        ...
        Assert.IsFalse(ProfileProjection.IsCircleAvailable(true, staleUpsert, delete));
        var revoked = ProfileProjection.MergeContact(null, incoming, []);
        Assert.AreEqual(0, revoked.Circles.Count);

        Assert.IsTrue(ProfileProjection.IsCircleAvailable(true, recreatedUpsert, delete));
        var joined = ProfileProjection.MergeContact(revoked, incoming, ["Friends"]);
        CollectionAssert.AreEqual(new[] { "Friends" }, joined.Circles);
    }

    [TestMethod]
    public void InitialProjection_AppliesAllCircleStateBeforeContactMembership()
    {
        const string source = "initial-device";
        ...
        var circle = Operation(
            DomainProjectionKinds.CircleUpsert,
            "friends",
            ProfileProjection.StableVersion(source, DomainProjectionKinds.CircleUpsert, "friends", circlePayload),
            circlePayload);
        var deletedCircle = Operation(
            DomainProjectionKinds.CircleDelete,
            "old",
            ProfileProjection.StableVersion(source, DomainProjectionKinds.CircleDelete, "old", ""),
            "");
        var contact = Operation(
            DomainProjectionKinds.ContactUpsert,
            "alice",
            ProfileProjection.StableVersion(source, DomainProjectionKinds.ContactUpsert, "alice", contactPayload),
            contactPayload);

        var ordered = ProfileProjection.OrderForApplication([contact, deletedCircle, circle]);

        Assert.IsTrue(ordered.Take(2).All(operation =>
            operation.Kind is DomainProjectionKinds.CircleUpsert or DomainProjectionKinds.CircleDelete));
        Assert.AreEqual(DomainProjectionKinds.ContactUpsert, ordered[2].Kind);

        var activeCircles = ordered
            .Take(2)
            .Where(operation => operation.Kind == DomainProjectionKinds.CircleUpsert)
            .Select(_ => "Friends")
            .ToList();
        var merged = ProfileProjection.MergeContact(null, Contact("alice", ["Friends"]), activeCircles);
    ...
    public void MemoryOperations_ApplyUpsertBeforeDelete()
    {
        var upsert = Operation(
            DomainProjectionKinds.MemoryUpsert,
            "memory-1",
            Version(10, "upsert"));
        var contact = Operation(
            DomainProjectionKinds.ContactUpsert,
        ...
        var ordered = ProfileProjection.OrderForApplication([delete, contact, upsert]);
    ...
    {
        var upsert = Operation(
            DomainProjectionKinds.TopicLineUpsert,
            "topic-1",
            Version(10, "upsert"));
        var delete = Operation(
            DomainProjectionKinds.TopicLineDelete,
            DomainProjectionEntityIds.TopicLine("topic-1", "line-1"),
            Version(20, "delete"),
            "");

        var ordered = ProfileProjection.OrderForApplication([delete, upsert]);
    ...
    public void RenameUpsert_IsAppliedBeforeItsOlderDeleteToPreserveReferences()
    {
        ...
        var renamePayload = System.Text.Json.JsonSerializer.Serialize(
            new CircleProjection(
                "Close Friends",
                false,
                [new CircleRenameProjection("Friends", deleteVersion)]));
        var delete = Operation(
            DomainProjectionKinds.CircleDelete,
            "friends",
            deleteVersion,
        ...
        var ordered = ProfileProjection.OrderForApplication([delete, rename]);
        ...
        ProfileProjection.RenameCircleReferences(profile, "Friends", "Close Friends");
    ...
    public void DeleteCircle_RemovesMembershipAndRevokesEveryVisibilitySurface()
    {
        ...
        ProfileProjection.DeleteCircleReferences(profile, "friends");
    ...
    [TestMethod]
    public void InitialBaseline_CannotBeatModernTombstone()
    {
        var first = ProfileProjection.StableVersion("device-a", DomainProjectionKinds.ContactUpsert, "alice", "{}");
        var divergent = ProfileProjection.StableVersion("device-b", DomainProjectionKinds.ContactUpsert, "alice", "{}");
        var tombstone = Version(DateTimeOffset.UtcNow.UtcTicks, "delete");

        Assert.AreNotEqual(first, divergent);
        Assert.IsTrue(ProjectionVersion.IsNewer(tombstone, first));
        Assert.IsFalse(ProjectionVersion.IsNewer(first, tombstone));
    ...
    public void AtomicWrite_RejectsUpsertOlderThanDeleteTombstone()
    {
        ...
        Assert.IsTrue(db.SaveProfileAndSyncState(
            ...
            [new MeshDb.SyncTombstoneWrite(
                DomainProjectionKinds.ContactDelete, "alice", deleteVersion)]));
        ...
        Assert.AreEqual(deleteVersion, db.GetSyncTombstoneVersion(
            DomainProjectionKinds.ContactDelete, "alice"));
    }
    ...
    public void DeleteAndTombstone_SurviveRestartTogether()
    {
        ...
        using (var db = MeshDb.Open(databasePath, key))
        {
            ...
            db.SaveProfileAndSyncState(
                Profile(contacts: []),
                [],
                [new MeshDb.SyncTombstoneWrite(DomainProjectionKinds.ContactDelete, "alice", version)]);
        }
        ...
        Assert.AreEqual(
            version,
            reopened.GetSyncTombstoneVersion(DomainProjectionKinds.ContactDelete, "alice"));
    }
            ...
            Assert.IsTrue(db.SaveProfileAndSyncState(
                ...
                [
                    new MeshDb.SyncCircleRenameWrite(
                        ...
                        [
                            new CircleRenameProjection("Friends", deleteVersion),
                            new CircleRenameProjection("Trusted", Version(10, "older-delete"))
                        ])
                ]));
        }
    ...
    private static ContactProjection Contact(string handle, IReadOnlyList<string> circles)
    ...
    private static ReplicationOperation Operation(
        string kind,
        string entityId,
        string version,
        string payload = "{}")
