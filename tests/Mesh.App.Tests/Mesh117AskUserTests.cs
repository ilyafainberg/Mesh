...
/// <summary>
/// Mesh 1.17 ask-user interaction tests for the owner/Me agent path.
///
/// The always-on class below exercises the durable store semantics the feature relies on
...
/// <c>MESH117_ASKUSER_TESTS</c> compilation constant so this file always compiles as-is.
/// See the report for the exact link + constant needed.
/// </summary>
[TestClass]
[DoNotParallelize]
public sealed class Mesh117AskUserTests
{
    ...
    // ------------------------------------------------------------------
    // Durable store: first-writer resolution wins, loser observes winner.
    // ------------------------------------------------------------------
    ...
    // durable pending state survives until explicitly cancelled/resolved.
    // ------------------------------------------------------------------
    ...
    {
        // App shutdown/restart must leave durable pending state (requirement 8).
        using (var db = MeshDb.Open(databasePath, key))
        {
            var store = new AskUserStore(db);
            store.CreateAsync(MakePendingPrompt("ask-durable")).GetAwaiter().GetResult();
        }
        SqliteConnection.ClearAllPools();
        ...
        {
            var store = new AskUserStore(db);
            var reloaded = store.GetAsync("ask-durable").GetAwaiter().GetResult();
    ...
    // ------------------------------------------------------------------
    // Restart exactly-once resume: the durable context fence admits a
...
#if MESH117_ASKUSER_TESTS
// ======================================================================
// Behavioural coverage for the pure, MAUI-free interaction types.
//
// To activate: add the following to tests\Mesh.App.Tests\Mesh.App.Tests.csproj
// (which this workstream is NOT permitted to edit) and define the constant:
//
//   <Compile Include="..\..\src\Mesh.App\Services\AskUserInteractionCoordinator.cs"
//            Link="Linked\AskUserInteractionCoordinator.cs" />
//   <DefineConstants>$(DefineConstants);MESH117_ASKUSER_TESTS</DefineConstants>
//
// The coordinator file only references Mesh.App.Domain and IAgentTool.cs (ToolArgs), both of
// which are already linked, so no further links are required for these tests.
// ======================================================================
[TestClass]
public sealed class Mesh117AskUserCoordinatorTests
{
