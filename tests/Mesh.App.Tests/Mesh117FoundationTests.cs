...
[TestClass]
[DoNotParallelize]
public sealed class Mesh117FoundationTests
{
    ...
    public void Asset_PageSummaries_DoesNotReturnContentBytes()
    {
        ...
        store.UpsertAsync(summary, content, false).GetAwaiter().GetResult();
    ...
    public void Asset_ContentHash_RoundTrip()
    {
        using var db = MeshDb.Open(databasePath, key);
        var store = new AssetStore(db);
        var content = "Hello, Mesh 1.17!"u8.ToArray();
        var hash = Convert.ToHexString(SHA256.HashData(content)).ToLowerInvariant();
    ...
    public void Asset_Delete_SetsTombstone()
    {
        ...
        store.UpsertAsync(summary, [1, 2, 3], false).GetAwaiter().GetResult();
        var tombstone = store.DeleteAsync(AssetKind.Skill, "w-1", "dev-1", false)
    ...
    [TestMethod]
    public void Asset_Upsert_CreatesOutboxEntry_WhenRequested()
    {
        using var db = MeshDb.Open(databasePath, key);
        var store = new AssetStore(db);
        var summary = MakeSkillRecord("outbox-1", 1, localOnly: false);
        store.UpsertAsync(summary, [1], createOutboxEntry: true).GetAwaiter().GetResult();

        var outbox = store.ListOutboxAsync(10).GetAwaiter().GetResult();
        Assert.AreEqual(1, outbox.Count);
        Assert.AreEqual("outbox-1", outbox[0].AssetId);
        Assert.AreEqual("upsert", outbox[0].Operation);
    }

    [TestMethod]
    public void Asset_Delete_CreatesOutboxEntry_WhenRequested()
    {
        using var db = MeshDb.Open(databasePath, key);
        var store = new AssetStore(db);
        var summary = MakeSkillRecord("del-outbox-1", 1, localOnly: false);
        store.UpsertAsync(summary, [1], createOutboxEntry: false).GetAwaiter().GetResult();
        store.DeleteAsync(AssetKind.Skill, "del-outbox-1", "dev-1", createOutboxEntry: true)
            .GetAwaiter().GetResult();

        var outbox = store.ListOutboxAsync(10).GetAwaiter().GetResult();
        Assert.AreEqual(1, outbox.Count);
        Assert.AreEqual("delete", outbox[0].Operation);
    }
    ...
    [TestMethod]
    public void Asset_LocalOnly_NeverProducesOutboxEntry()
    {
        using var db = MeshDb.Open(databasePath, key);
        var store = new AssetStore(db);
        var local = MakeSkillRecord("local-1", 1, localOnly: true);
        store.UpsertAsync(local, [1], createOutboxEntry: true).GetAwaiter().GetResult();
        store.DeleteAsync(AssetKind.Skill, "local-1", "dev-1", createOutboxEntry: true)
            .GetAwaiter().GetResult();

        var outbox = store.ListOutboxAsync(10).GetAwaiter().GetResult();
        Assert.AreEqual(0, outbox.Count, "LocalOnly assets must never enter the outbox.");
    }
    ...
    public void Asset_ApplyRemoteUpsert_RejectsOlderVersion()
    {
        ...
        store.UpsertAsync(v2, [2], false).GetAwaiter().GetResult();
    ...
    public void Asset_ApplyRemoteUpsert_AcceptsNewerVersion()
    {
        ...
        store.UpsertAsync(v1, [1], false).GetAwaiter().GetResult();
    ...
    public void Asset_ApplyRemoteDelete_RejectsLowerVersion()
    {
        ...
        store.UpsertAsync(v5, [], false).GetAwaiter().GetResult();
    ...
    public void Asset_Pagination_ReturnsCorrectPage()
    {
        ...
        for (int i = 0; i < 10; i++)
            store.UpsertAsync(MakeSkillRecord($"pg-{i:D3}", 1), [], false)
                .GetAwaiter().GetResult();
    ...
    [TestMethod]
    public void Asset_Outbox_DequeueIncrements_MarkSuccessDeletes()
    {
        using var db = MeshDb.Open(databasePath, key);
        var store = new AssetStore(db);
        store.UpsertAsync(MakeSkillRecord("oq-1", 1), [], createOutboxEntry: true)
            .GetAwaiter().GetResult();

        var before = store.ListOutboxAsync(5).GetAwaiter().GetResult();
        Assert.AreEqual(0, before[0].Attempts);

        var dequeued = store.DequeueOutboxAsync(5).GetAwaiter().GetResult();
        Assert.AreEqual(1, dequeued.Count);

        var afterDequeue = store.ListOutboxAsync(5).GetAwaiter().GetResult();
        Assert.AreEqual(1, afterDequeue[0].Attempts);

        store.MarkOutboxAttemptAsync(dequeued[0].OperationId, success: true, null)
            .GetAwaiter().GetResult();
        var afterSuccess = store.ListOutboxAsync(5).GetAwaiter().GetResult();
        Assert.AreEqual(0, afterSuccess.Count, "Successful outbox entry should be removed.");
    }

    [TestMethod]
    public void Asset_Outbox_MarkFailure_StoresError()
    {
        using var db = MeshDb.Open(databasePath, key);
        var store = new AssetStore(db);
        store.UpsertAsync(MakeSkillRecord("oq-fail-1", 1), [], createOutboxEntry: true)
            .GetAwaiter().GetResult();

        var item = store.DequeueOutboxAsync(1).GetAwaiter().GetResult()[0];
        store.MarkOutboxAttemptAsync(item.OperationId, success: false, "network timeout")
            .GetAwaiter().GetResult();
    ...
    [TestMethod]
    public void Schema_Foundation117_IdempotentAndExistingDbReadable()
    {
        // First open creates all tables.
        using (var db = MeshDb.Open(databasePath, key))
        {
            var store = new AssetStore(db);
            store.UpsertAsync(MakeSkillRecord("idem-1", 1), [42], false)
                .GetAwaiter().GetResult();
        }
        SqliteConnection.ClearAllPools();

        // Second open calls CreateFoundation117Schema again (idempotent IF NOT EXISTS).
    ...
    public void Asset_UpsertHashMismatch_Throws()
    {
        ...
        Assert.ThrowsException<InvalidOperationException>(() =>
            store.UpsertAsync(tampered, [1, 2, 3], false).GetAwaiter().GetResult());
    }
    ...
    public void Asset_EmptyContent_StoresRowAndHash_ThenReplaces()
    {
        ...
        store.UpsertAsync(MakeSkillRecord("empty-1", 1), [], false).GetAwaiter().GetResult();
        ...
        store.UpsertAsync(MakeSkillRecord("empty-1", 2), content, false).GetAwaiter().GetResult();
    ...
    [TestMethod]
    public void AssetOperationId_IsStableAndVersioned()
    {
        var a = AssetOperationId.Create(AssetKind.Skill, "x", "upsert", 1, "dev-1");
        var b = AssetOperationId.Create(AssetKind.Skill, "x", "upsert", 1, "dev-1");
        Assert.AreEqual(a, b, "The same operation must yield the same id.");
        Assert.AreEqual(64, a.Length, "SHA-256 hex is 64 chars.");

        Assert.AreNotEqual(
            a, AssetOperationId.Create(AssetKind.Skill, "x", "upsert", 2, "dev-1"),
            "A newer version must yield a new id.");
        Assert.AreNotEqual(
            a, AssetOperationId.Create(AssetKind.Skill, "x", "delete", 1, "dev-1"));
        Assert.AreNotEqual(
            a, AssetOperationId.Create(AssetKind.Knowledge, "x", "upsert", 1, "dev-1"));
    }

    [TestMethod]
    public void Asset_OutboxDuplicateOp_IsIdempotent_NewerVersionGetsNewRow()
    {
        using var db = MeshDb.Open(databasePath, key);
        var store = new AssetStore(db);

        store.UpsertAsync(MakeSkillRecord("dup-1", 1, localOnly: false), [1], createOutboxEntry: true)
            .GetAwaiter().GetResult();
        store.UpsertAsync(MakeSkillRecord("dup-1", 1, localOnly: false), [1], createOutboxEntry: true)
            .GetAwaiter().GetResult();

        Assert.AreEqual(1, store.ListOutboxAsync(10).GetAwaiter().GetResult().Count,
            "A duplicate operation must not create a second outbox row.");

        store.UpsertAsync(MakeSkillRecord("dup-1", 2, localOnly: false), [2], createOutboxEntry: true)
            .GetAwaiter().GetResult();

        Assert.AreEqual(2, store.ListOutboxAsync(10).GetAwaiter().GetResult().Count,
            "A newer revision must enqueue a distinct outbox row.");
    }

    [TestMethod]
    public void Asset_ConcurrentDequeue_SelectionAndIncrementAreAtomic()
    {
        using var db = MeshDb.Open(databasePath, key);
        var store = new AssetStore(db);
        for (int i = 0; i < 20; i++)
            store.UpsertAsync(
                    MakeSkillRecord($"dq-{i:D2}", 1, localOnly: false), [(byte)i],
                    createOutboxEntry: true)
                .GetAwaiter().GetResult();
        ...
        var tasks = Enumerable.Range(0, workers).Select(_ => Task.Run(() =>
        {
            start.Wait();
            for (int k = 0; k < 10; k++)
                foreach (var item in store.DequeueOutboxAsync(5).GetAwaiter().GetResult())
                    seen.Add(item.OperationId);
        })).ToArray();

        start.Set();
        Assert.IsTrue(Task.WaitAll(tasks, TimeSpan.FromSeconds(60)));

        var all = store.ListOutboxAsync(500).GetAwaiter().GetResult();
        long totalAttempts = all.Sum(i => (long)i.Attempts);
        Assert.AreEqual(seen.Count, totalAttempts,
            "Every dequeued row must be counted exactly once (no lost increments).");
    }
