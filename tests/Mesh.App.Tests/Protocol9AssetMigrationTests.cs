...
/// <summary>
/// Behavioral, bounded-memory tests for the Protocol 9 capability-asset migration and the
...
///   * <see cref="MeshDb"/>/<see cref="AssetStore"/> - reliable asset persistence, tombstones, outbox.
///
...
/// bounded blob only after every asset is reliable, then hydrate from the tables), so the tests
...
[TestClass]
[DoNotParallelize]
public sealed class Protocol9AssetMigrationTests
{
    ...
    public void Initialize()
    {
        directory = Path.Combine(
            ...
            "protocol9-asset-migration-tests",
    ...
    public void Migration_MovesLegacyAssetsToTables_AndLeavesBoundedBlob()
    {
        ...
        using (var db = MeshDb.Open(databasePath, key))
        {
            ...
            // Everything is reliable in the asset tables and hydrates back losslessly.
    ...
    public void Migration_RollsBackOnFailure_LeavesLegacyBlob_ThenRecovers()
    {
        ...
        using (var db = MeshDb.Open(databasePath, key))
        {
            ...
            Assert.ThrowsException<InvalidOperationException>(() =>
                db.UpsertAsset(corrupt, content));

            // Because the rewrite only happens after every asset is reliable, the legacy blob is intact.
    ...
    public void RapidMutations_FinalRevisionIsPersisted_NoLostUpdates()
    {
        ...
        for (var i = 1; i <= iterations; i++)
        {
            ...
            store.UpsertAsync(record, content).GetAwaiter().GetResult();
        }
        sw.Stop();
    ...
    public void ProfileBlob_AsyncWritePath_PersistsLatestJson()
    {
        ...
        Assert.AreEqual("Name 49", db.LoadProfile()!.DisplayName, "The last scheduled blob write must be the reliable one.");
    }
    ...
    /// absent from the tables), then rewrite the now-bounded blob once every asset is reliable.
    /// </summary>
    private static void Migrate(MeshDb db, MeshProfile profile, bool localOnly)
    {
        ...
        db.SaveProfile(profile); // bounded blob, only after every asset is reliable
    }
    ...
    /// <summary>Writes a legacy profile blob that still embeds its assets (the migration input).</summary>
