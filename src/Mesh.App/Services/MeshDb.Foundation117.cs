...
// Foundation 1.17 schema initialisation and CRUD methods for ask-user prompts,
// suspended contexts, assets, asset content and asset outbox.
public sealed partial class MeshDb
{
    private static readonly JsonSerializerOptions Foundation117Json =
        new(JsonSerializerDefaults.Web);

    // Called from CreateSchema() after all existing migrations.
    internal void CreateFoundation117Schema()
    {
        Exec("""
        ...
        // Re-open safety for databases created by an earlier 1.17 development schema that
        // predates the deterministic outbox columns.
        AddColumnIfMissing("asset_outbox", "version", "INTEGER NOT NULL DEFAULT 1");
        AddColumnIfMissing("asset_outbox", "source_device_id", "TEXT");
    }
    ...
    private static void BindAskUserPrompt(SqliteCommand cmd, AskUserPrompt p)
    {
        ...
        cmd.Parameters.AddWithValue("$options",
            JsonSerializer.Serialize(p.Options, Foundation117Json));
    ...
    private static AskUserPrompt ReadAskUserPrompt(SqliteDataReader r)
    {
        var options = JsonSerializer.Deserialize<List<AskUserOption>>(
            r.GetString(r.GetOrdinal("options_json")), Foundation117Json)
            ?? [];
    ...
    /// even when the content is empty. When requested and the asset is not
    /// <see cref="AssetRecord.LocalOnly"/>, a deterministic outbox entry is added in the
    /// same transaction.
    /// </summary>
    public void UpsertAsset(AssetRecord summary, byte[] content, bool createOutboxEntry)
    {
        ...
        if (createOutboxEntry && !normalized.LocalOnly)
            InsertAssetOutboxInTransaction(
                tx, normalized.Kind, normalized.Id, "upsert",
                normalized.Version, normalized.SourceDeviceId);
        tx.Commit();
    }
    ...
    /// tombstone is returned and, when requested and not local-only, drives the outbox entry.
    /// </summary>
    public AssetRecord DeleteAsset(
        AssetKind kind, string id, string sourceDeviceId, bool createOutboxEntry)
    {
        ...
        if (createOutboxEntry && !localOnly)
            InsertAssetOutboxInTransaction(
                tx, kind, id, "delete", newVersion, sourceDeviceId);
        tx.Commit();
        return tombstone;
    }
    ...
    private void InsertAssetOutboxInTransaction(
        SqliteTransaction tx, AssetKind kind, string assetId, string operation,
        int version, string? sourceDeviceId)
    {
        using var cmd = conn.CreateCommand();
        cmd.Transaction = tx;
        cmd.CommandText = """
            ...
            VALUES($opId, $kind, $assetId, $op, $version, $device, 0, NULL, $created)
            ON CONFLICT(operation_id) DO NOTHING;
            """;
        cmd.Parameters.AddWithValue(
            "$opId", AssetOperationId.Create(kind, assetId, operation, version, sourceDeviceId));
        cmd.Parameters.AddWithValue("$kind", kind.ToString());
    ...
    public IReadOnlyList<AssetOutboxItem> ListAssetOutbox(int max)
    {
        using var cmd = conn.CreateCommand();
        cmd.CommandText = AssetOutboxSelect;
        cmd.Parameters.AddWithValue("$max", max);
        using var r = cmd.ExecuteReader();
        var result = new List<AssetOutboxItem>();
        while (r.Read()) result.Add(ReadAssetOutboxItem(r));
        return result;
    }

    /// <summary>
    /// Selects up to <paramref name="max"/> outbox items and increments their attempt
    /// counters within a single transaction, so selection and increment cannot interleave.
    /// The returned items carry their pre-increment attempt counts.
    /// </summary>
    public IReadOnlyList<AssetOutboxItem> DequeueAssetOutbox(int max)
    {
        using var tx = conn.BeginTransaction(deferred: false);
        var items = new List<AssetOutboxItem>();
        using (var sel = conn.CreateCommand())
        {
            sel.Transaction = tx;
            sel.CommandText = AssetOutboxSelect;
            sel.Parameters.AddWithValue("$max", max);
            using var r = sel.ExecuteReader();
            while (r.Read()) items.Add(ReadAssetOutboxItem(r));
        }
        if (items.Count == 0)
        {
            tx.Rollback();
        ...
        foreach (var item in items)
        {
            ...
            upd.CommandText = """
                ...
                """;
            upd.Parameters.AddWithValue("$id", item.OperationId);
            upd.ExecuteNonQuery();
        }
        tx.Commit();
        return items;
    ...
    /// <summary>
    /// Marks an outbox operation. On success the row is deleted;
    /// on failure the error text is stored and the operation remains queued.
    /// </summary>
    public void MarkAssetOutboxAttempt(string operationId, bool success, string? error)
    {
        using var cmd = conn.CreateCommand();
        if (success)
        {
            cmd.CommandText =
                "DELETE FROM asset_outbox WHERE operation_id = $id;";
    ...
    private static AssetOutboxItem ReadAssetOutboxItem(SqliteDataReader r)
    {
        var rawKind = r.GetString(1);
        var kind = Enum.TryParse<AssetKind>(rawKind, ignoreCase: true, out var k)
            ? k : AssetKind.Skill;
        return new AssetOutboxItem(
            OperationId: r.GetString(0),
            Kind: kind,
            AssetId: r.GetString(2),
            Operation: r.GetString(3),
            Version: r.GetInt32(4),
            SourceDeviceId: r.IsDBNull(5) ? null : r.GetString(5),
            Attempts: r.GetInt32(6),
            LastError: r.IsDBNull(7) ? null : r.GetString(7),
            CreatedAt: ParseAt(r.GetString(8)));
    }
