...
public sealed class TopicAttachmentAssembler : IDisposable
{
    ...
    public TopicAttachmentAssembler(string? root = null)
    {
        this.root = root ?? Path.Combine(StoragePaths.Root, "Temp", "device-topic-staging");
    ...
    public bool TryAdd(string sourceDeviceId, AttachmentChunkPayload chunk, out string error)
    {
        ...
        {
            error = "attachment staging is disposed";
            return false;
        }
        ...
        if (!runs.TryGetValue(runKey, out var run))
        {
            lock (runCreationGate)
            {
                if (!runs.TryGetValue(runKey, out run))
                {
                    ...
                    {
                        error = "attachment staging is full";
                        return false;
                    }
